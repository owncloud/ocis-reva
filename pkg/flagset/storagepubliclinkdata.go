...
// StoragePublicLinkDataWithConfig applies cfg to the root flagset
func StoragePublicLinkDataWithConfig(cfg *config.Config) []cli.Flag {
	return []cli.Flag{
		&cli.BoolFlag{
		...
		&cli.StringFlag{
			...
			EnvVars:     []string{"REVA_STORAGE_PUBLIC_LINK_DATA_DEBUG_ADDR"},
			Destination: &cfg.Reva.StoragePublicLinkData.DebugAddr,
		},
		&cli.StringFlag{
			Name:        "debug-token",
			Value:       "",
		...
		&cli.StringFlag{
			...
			EnvVars:     []string{"REVA_STORAGE_PUBLIC_LINK_DATA_NETWORK"},
			Destination: &cfg.Reva.StoragePublicLinkData.Network,
		},
		&cli.StringFlag{
			Name:        "protocol",
			Value:       "http",
			Usage:       "protocol for reva service, can be 'http' or 'grpc'",
			EnvVars:     []string{"REVA_STORAGE_PUBLIC_LINK_DATA_PROTOCOL"},
			Destination: &cfg.Reva.StoragePublicLinkData.Protocol,
		},
		&cli.StringFlag{
			Name:        "addr",
			Value:       "0.0.0.0:9199",
			Usage:       "Address to bind reva service",
			EnvVars:     []string{"REVA_STORAGE_PUBLIC_LINK_DATA_ADDR"},
			Destination: &cfg.Reva.StoragePublicLinkData.Addr,
		},
		&cli.StringFlag{
			Name:        "url",
			Value:       "localhost:9198",
			Usage:       "URL to use for the reva service",
			EnvVars:     []string{"REVA_STORAGE_PUBLIC_LINK_DATA_URL"},
			Destination: &cfg.Reva.StoragePublicLinkData.URL,
		},
		&cli.StringSliceFlag{
			Name:    "service",
			Value:   cli.NewStringSlice("dataprovider"),
			Usage:   "--service dataprovider [--service otherservice]",
			EnvVars: []string{"REVA_STORAGE_PUBLIC_LINK_DATA_SERVICES"},
		},
		&cli.StringFlag{
			Name:        "driver",
			Value:       "owncloud",
			Usage:       "storage driver, eg. local, eos, owncloud or s3",
			EnvVars:     []string{"REVA_STORAGE_PUBLIC_LINK_DATA_DRIVER"},
			Destination: &cfg.Reva.StoragePublicLinkData.Driver,
		},
		&cli.StringFlag{
			Name:        "prefix",
			Value:       "data",
			Usage:       "prefix for the http endpoint, without leading slash",
			EnvVars:     []string{"REVA_STORAGE_PUBLIC_LINK_DATA_PREFIX"},
			Destination: &cfg.Reva.StoragePublicLinkData.Prefix,
		},
		&cli.StringFlag{
			Name:        "temp-folder",
			Value:       "/var/tmp/",
			Usage:       "temp folder",
			EnvVars:     []string{"REVA_STORAGE_PUBLIC_LINK_DATA_TEMP_FOLDER"},
			Destination: &cfg.Reva.StoragePublicLinkData.TempFolder,
		},
